...
@Component
@EnableScheduling
public class TaskCatData {
    private final static Logger log = LoggerFactory.getLogger(TaskCatData.class);
    ...
    /**
     * @Description: 可爱猫回调订单的数据补充
     * <p>
     *     每1每秒运行一次
     *     1.可爱猫的to_wxid值找出小微的主键ID，并且填充
     *
     * </p>
     * @author yoko
...
//    @Scheduled(cron = "5 * * * * ?")
    ...
    public void catDataWorkType() throws Exception{
//        log.info("----------------------------------TaskCatData.catDataWorkType()----start");
...
//                log.info("----------------------------------TaskCatData.catDataWorkType()----end");
            }catch (Exception e){
                log.error(String.format("this TaskCatData.catDataWorkType() is error , the dataId=%s !", data.getId()));
                e.printStackTrace();
                // 更新此次task的状态：更新成失败
